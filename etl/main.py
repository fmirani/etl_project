...
def set_environment():
    '''
    Function to set up environment upon first execution
    '''
    conf = loadconfig()
    ...
    if not os.path.exists(os.path.join(data_path, "config/config.yaml")):
        ...
        logger.info("Configuration file exists already")
    ...
    # If the data file doen't exist, initiate dummy file
    yt_path = os.path.join(data_path, conf["youtube"]["history_file"])
    if not os.path.exists(yt_path):
        Path(yt_path).touch()

    # If the data file doen't exist, initiate dummy file
    nf_path = os.path.join(data_path, conf["netflix"]["history_file"])
...
def full_run():
    ...
    logger.info("Going to set environment first")
    set_environment()
